...
export class DEMA extends Strategy<'DEMA'> {
  ...
  protected init(): void {
    this.addIndicator('DEMA', { weight: this.strategySettings.weight });
    this.addIndicator('SMA', { weight: this.strategySettings.weight });
  }
  ...
  protected onCandleAfterWarmup(candle: Candle): void {
    ...
    const message =
      '@ ' + price.toFixed(8) + ' (' + resDEMA.toFixed(5) + '/' + diff.toFixed(5) + ')';
