...
import { StrategyAlreadyInitializedError } from '../errors/strategyAlreadyInitialized.error';
...
export abstract class Strategy<T extends StrategyNames> extends EventEmitter {
  ...
  public onTradeCompleted(trade: TradeCompleted) {
    if (
      this.pendingTriggerAdvice &&
      trade.action === 'sell' &&
      this.pendingTriggerAdvice === trade.adviceId
    ) {
  ...
  protected advice(direction: RecommendedAdvice) {
    ...
    if (isObject(direction)) {
      if (newDirection === 'short') {
        logger.warn(
          'Strategy adviced a stop on not long, this is not supported. As such the stop is ignored',
        );
      }
      if (newDirection === 'long') {
        ...
        advice.trigger = {
          ...direction.trigger,
          ...(trailPercentage &&
            !trailValue && { trailValue: +Big(trailPercentage).div(100).mul(this.candle.close) }),
        };
  ...
  private warmup(candle: Candle) {
    ...
    const isPremature =
      config.getWatch().mode === 'realtime' &&
      isBefore(candle.start, subMinutes(stateManager.getStartTime(), this.candleSize));
