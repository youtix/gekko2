...
describe('TMA Strategy', () => {
  ...
  it('should emit long advice when short > medium > long', () => {
    strategy.onTimeframeCandleAfterWarmup({ candle: bucket, tools } as any, 10, 5, 2);
  ...
  it('should emit short advice when short < medium and medium > long', () => {
    strategy.onTimeframeCandleAfterWarmup({ candle: bucket, tools } as any, 3, 5, 2);
    expect(advices).toEqual([{ type: 'STICKY', side: 'SELL', amount: 1, symbol: 'BTC/USDT' }]);
  });

  it('should emit short advice when short > medium and medium < long', () => {
    strategy.onTimeframeCandleAfterWarmup({ candle: bucket, tools } as any, 5, 3, 7);
    expect(advices).toEqual([{ type: 'STICKY', side: 'SELL', amount: 1, symbol: 'BTC/USDT' }]);
  });

  it('should not emit advice when no clear trend', () => {
    strategy.onTimeframeCandleAfterWarmup({ candle: bucket, tools } as any, 5, 5, 5);
