...
describe('ADXRibbon', () => {
  const ribbon = new ADXRibbon({ count: 19, start: 1, step: 1 });
  it.each`
    candle                                                                                      | expected
    ...
    ${{ close: 58, open: 65, high: 68.65939501847261, low: 54.340604981527385, volume: 510 }}   | ${null}
    ${{ close: 17, open: 58, high: 60.01918882495998, low: 14.980811175040019, volume: 878 }}   | ${null}
    ${{ close: 90, open: 17, high: 93.37632122668938, low: 13.623678773310617, volume: 150 }}   | ${null}
    ${{ close: 87, open: 90, high: 91.79876207396401, low: 85.20123792603599, volume: 914 }}    | ${null}
    ${{ close: 86, open: 87, high: 90.67917429835809, low: 82.32082570164191, volume: 582 }}    | ${null}
    ${{ close: 99, open: 86, high: 103.90666224637769, low: 81.09333775362231, volume: 804 }}   | ${null}
    ${{ close: 3, open: 99, high: 100.63598126886475, low: 1.3640187311352432, volume: 436 }}   | ${null}
    ${{ close: 70, open: 3, high: 74.01339408473842, low: -1.0133940847384295, volume: 747 }}   | ${null}
    ${{ close: 1, open: 70, high: 70.62974231530183, low: 0.37025768469818, volume: 722 }}      | ${null}
    ${{ close: 27, open: 1, high: 30.879348918826008, low: -2.879348918826009, volume: 221 }}   | ${null}
    ${{ close: 9, open: 27, high: 29.207668220474442, low: 6.792331779525556, volume: 148 }}    | ${null}
    ${{ close: 92, open: 9, high: 96.46225023362183, low: 4.5377497663781705, volume: 331 }}    | ${null}
    ${{ close: 68, open: 92, high: 94.82774764949542, low: 65.17225235050458, volume: 338 }}    | ${{ results: [0, 88.378476756182, 66.55481808559794, 50.86258563024911, 40.535652521530025, 33.71309693815243, 29.950879275831127, 28.770921515533672, 28.41821665179067, 28.357283448033538, 29.20851685922201, 30.18881971132567, 31.445441088743998, 32.246836227285925, 33.41109199969273, 34.00427155195924, 34.09742747499928, 34.3340616684213, 34.4244638087148] }}
    ${{ close: 9, open: 68, high: 69.94866467256739, low: 7.051335327432617, volume: 823 }}     | ${{ results: [100, 71.94040328553587, 56.007918033645865, 44.941236134812556, 37.17929926895017, 31.780359881783276, 28.728532182629717, 27.819388874201778, 27.612946871844642, 27.652389754637003, 28.52527480647244, 29.51839774103, 30.766392193993628, 31.59396265817166, 32.77064178655686, 33.40512095844067, 33.562271814013656, 33.853990566994675, 33.99944907298064] }}
  `('returns $expected when candle.close = $candle.close', ({ candle, expected }) => {
    ...
    else expect(result.results[1]).toBeCloseTo(expected.results[1], 13);
  });
});
