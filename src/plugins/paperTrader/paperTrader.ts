...
export class PaperTrader extends Plugin {
  activeStopTrigger?: ActiveStopTrigger;
  balance: number;
  candle?: Candle;
  exposed: boolean;
  fee: number;
  portfolio: { asset: number; currency: number };
  price: number;
  propogatedTrades: number;
  propogatedTriggers: number;
  rawFee: number;
  trades: number;
  tradeId?: string;
  warmupCandle?: Candle;
  ...
  private createTrigger(advice: Advice) {
    ...
    if (trigger && trigger.type === 'trailingStop') {
      ...
      logger.warn(
        `[Papertrader] Gekko does not know trigger with type "${trigger?.type}".. Ignoring stop.`,
      );
    }
  }
  ...
  private updatePosition(recommendation: 'short' | 'long') {
    ...
    if (recommendation === 'long') {
      ...
      this.portfolio.asset += +Big(this.portfolio.currency)
        .div(this.price)
        .mul(this.fee)
      ...
      this.portfolio.currency += +Big(this.portfolio.asset)
        .mul(this.price)
        .mul(this.fee)
        .round(8, Big.roundDown);
  ...
  public static getStaticConfiguration() {
    return {
      ...
      dependencies: [],
      inject: [],
      eventsHandlers: filter(Object.getOwnPropertyNames(PaperTrader.prototype), (p) =>
        p.startsWith('on'),
      ),
      eventsEmitted: [
        PORTFOLIO_CHANGE_EVENT,
