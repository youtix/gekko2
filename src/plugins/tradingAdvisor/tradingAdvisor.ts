...
import { StrategyNotFoundError } from '@errors/strategyNotFound.error';
...
export class TradingAdvisor extends Plugin {
  ...
  constructor({ candleSize, strategyName, historySize }: TradingAdvisorConfiguration) {
    ...
    const relayers = filter(Object.getOwnPropertyNames(TradingAdvisor.prototype), (p) =>
      p.startsWith('relay'),
    );
  ...
  // --- BEGIN INTERNALS ---
  private async setUpStrategy(strategyName: string, candleSize: number, historySize: number) {
  ...
  private setUpListeners() {
    this.strategy
      ...
      .on(TRADE_COMPLETED_EVENT, this.relayTradeCompleted);
  }
  ...
  private relayTradeCompleted(event: unknown) {
    this.deferredEmit(TRADE_COMPLETED_EVENT, event);
  }
  ...
  public static getStaticConfiguration() {
    return {
      name: TradingAdvisor.name,
      schema: tradingAdvisorSchema,
      modes: ['realtime', 'backtest'],
      dependencies: [],
      inject: [],
      eventsHandlers: filter(Object.getOwnPropertyNames(TradingAdvisor.prototype), (p) =>
        p.startsWith('on'),
      ),
      eventsEmitted: [
        ADVICE_EVENT,
