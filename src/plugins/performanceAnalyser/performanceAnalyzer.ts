...
import {
  ...
  SingleRoundTrip,
  Start,
} from "./performanceAnalyzer.types";
import {
  logFinalize,
  logImpossibleToProcessReport,
  logRoundtrip,
  logTrade,
} from "./performanceAnalyzer.utils";
...
export class PerformanceAnalyzer extends Plugin {
  ...
  private registerRoundtripPart(trade: TradeCompleted): void {
    ...
    if (trade.action === "buy") {
      ...
      this.roundTrip.entry = {
        date: trade.date,
        price: trade.price,
        total: +Big(trade.portfolio.asset)
          .mul(trade.price)
          .plus(trade.portfolio.currency),
      };
  ...
  private handleCompletedRoundtrip(): void {
    ...
    const roundtrip: RoundTrip = {
      ...
      profit: +Big(100)
        .mul(this.roundTrip.exit.total)
        .div(this.roundTrip.entry.total)
        .minus(100),

      duration: differenceInMilliseconds(
        this.roundTrip.exit.date,
        this.roundTrip.entry.date,
      ),
    };
  ...
  private calculateReportStatistics() {
    ...
    const relativeProfit = +Big(this.balance)
      .div(this.start.balance)
      .mul(100)
      .minus(100);
    const relativeYearlyProfit = +Big(relativeProfit).div(timespan.years || 1);

    const percentExposure = +Big(this.exposure).div(
      differenceInMilliseconds(this.dates.end, this.dates.start),
    );
    ...
    const downside =
      downsideLosses.length > 0
        ? +Big(this.trades)
            .div(tradeCount)
            .sqrt()
            .mul(percentile(downsideLosses, 0.25))
        : 0;

    const positiveRoundtrips = this.roundTrips.filter(
      (roundTrip) => roundTrip.pnl > 0,
    );

    const ratioRoundTrips =
      this.roundTrips.length > 0
        ? +Big(positiveRoundtrips.length)
            .div(this.roundTrips.length)
            .mul(100)
            .round(4)
        : 100;

    const market = +Big(this.endPrice)
      .minus(this.startPrice)
      .div(this.startPrice)
      .mul(100);
  ...
  public static getStaticConfiguration() {
    return {
      ...
      eventsHandlers: filter(
        Object.getOwnPropertyNames(PerformanceAnalyzer.prototype),
        (p) => p.startsWith("on"),
      ),
      eventsEmitted: [
        PERFORMANCE_REPORT_EVENT,
