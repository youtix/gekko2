...
import { percentile, stdev } from '../../utils/math/math';
...
export class PerformanceAnalyzer extends Plugin {
  ...
  private handleCompletedRoundtrip(): void {
    ...
    logRoundtrip(roundtrip);

    this.deferredEmit(ROUNDTRIP_EVENT, roundtrip);
  ...
  private calculateReportStatistics() {
    ...
    const downside =
      downsideLosses.length > 0
        ? +Big(this.trades).div(tradeCount).sqrt().mul(percentile(downsideLosses, 0.25))
        : 0;
    ...
    const ratioRoundTrips =
      this.roundTrips.length > 0
        ? +Big(positiveRoundtrips.length).div(this.roundTrips.length).mul(100).round(4)
        : 100;
  ...
  public static getStaticConfiguration() {
    return {
      ...
      dependencies: [],
      inject: [],
      eventsHandlers: filter(Object.getOwnPropertyNames(PerformanceAnalyzer.prototype), (p) =>
        p.startsWith('on'),
      ),
