...
import { TRADE_COMPLETED_EVENT } from '@plugins/tradingAdvisor/tradingAdvisor.const';
import {
...
export class Trader extends Plugin {
  ...
  public onAdvice(advice: Advice) {
    ...
    if (this.order) {
      ...
      if (this.cancellingOrder) {
        logger.info(
          `[TRADER] Ignoring advice: already cancelling previous ${this.order.getSide()} order`,
        );
        return;
      }
  ...
  private cancelOrder(id: string, advice: Advice, callback: () => void) {
    ...
    this.order.once(ORDER_COMPLETED_EVENT, async () => {
      ...
      this.cancellingOrder = false;
      this.deferredEmit(TRADE_CANCELED_EVENT, {
        id,
        adviceId: advice.id,
        date: Date.now(),
      });
  ...
  private async createOrder(side: Action, amount: number, advice: Advice, id: string) {
    ...
    this.order.on(ORDER_STATUS_CHANGED_EVENT, status =>
      logger.debug(`[TRADER] status changed: ${status}`),
    );
    this.order.on(ORDER_ERRORED_EVENT, reason => {
      ...
      this.deferredEmit(TRADE_ERRORED_EVENT, { id, adviceId: advice.id, date: Date.now(), reason });
    });
    this.order.on(ORDER_COMPLETED_EVENT, async () => {
      try {
        ...
        if (error instanceof Error) {
          ...
          return this.deferredEmit(TRADE_ERRORED_EVENT, {
            id,
            adviceId: advice.id,
            date: Date.now(),
  ...
  private async handleOrderCompletedEvent(advice: Advice, id: string) {
    if (!this.order)
      throw new PluginError('trader', 'Missing order when handling order completed event');
