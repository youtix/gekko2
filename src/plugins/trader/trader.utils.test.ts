...
describe('processCostAndPrice', () => {
  it.each`
    ...
    ${'apply fee markup for BUY side'}    | ${'BUY'}  | ${100} | ${2}   | ${0.75}    | ${{ effectivePrice: 100.75, cost: 1.5 }}
    ${'apply fee discount for SELL side'} | ${'SELL'} | ${100} | ${2}   | ${0.5}     | ${{ effectivePrice: 99.5, cost: 1 }}
    ${'handle zero fee without warning'}  | ${'SELL'} | ${250} | ${3}   | ${0}       | ${{ effectivePrice: 250, cost: 0 }}
  `('should $description', ({ side, price, amount, feePercent, expected }) => {
    const result = processCostAndPrice(side, price, amount, feePercent);
    expect(warningMock).not.toHaveBeenCalled();
    expect(result).toEqual(expected);
  });

  it('should assume no fees and emit warning when feePercent is not provided', () => {
    const result = processCostAndPrice('BUY', 150, 3);
    expect(warningMock).toHaveBeenCalledWith('trader', 'Exchange did not provide fee information, assuming no fees..');
    expect(result).toEqual({ effectivePrice: 150, cost: 450 });
  });
});
