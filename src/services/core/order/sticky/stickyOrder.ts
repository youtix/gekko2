...
export class StickyOrder extends BaseOrder {
  ...
  public async createSummary() {
    if (!this.isOrderCompleted())
      throw new OrderError("Order is not completed");
    ...
    logger.debug(
      [
        `[ORDER] ${myTrades.length} trades used to fill sticky order.`,
    ...
    return {
      ...
      feePercent: feePercents.length
        ? weightedMean(feePercents, amounts)
        : undefined,
      side: this.side,
  ...
  private async checkOrder() {
    logger.debug(`[ORDER] Starting checking order ${this.id} status`);
  ...
  private async move() {
    logger.debug(`[ORDER] Starting moving order ${this.id}`);
  ...
  private updateTransactionPartialFilledAmount(id: string, filled = 0) {
    const lastFilledValueOfCurrentTransaction =
      find(this.transactions, { id })?.filled ?? 0;
  ...
  // Overrided functions
  protected handleCreateOrderSuccess({
    id,
    status,
    filled,
    price,
    ...
    logger.debug(
      [
        `[ORDER] Order ${id} created with success.`,
        `Status: ${status};`,
        `Filled: ${filled};`,
        `Price: ${price};`,
        `Remaining: ${remaining};`,
    ...
    this.transactions = reject(
      this.transactions,
      (t) => t.id === this.id && t.filled === 0,
    );
    ...
    logger.warn(
      `[ORDER] Order creation succeeded, but unknown status (${status}) returned`,
    );
  ...
  protected handleCancelOrderSuccess({
    id,
    status,
    filled,
    remaining,
    ...
    logger.debug(
      [
        `[ORDER] Order ${id} canceled with success.`,
    ...
    const totalFilledOfAllTransactions = +Big(
      sumBy(this.transactions, "filled"),
    ).add(filled ?? 0);
  ...
  protected async handleFetchOrderSuccess({
    id,
    status,
    filled,
    price,
    ...
    logger.debug(
      [
        `[ORDER] Order ${id} fetched with success.`,
        `Order is moving: ${this.moving};`,
        `Status: ${status};`,
        `Filled: ${filled};`,
        `Price: ${price};`,
    ...
    if (status === "open") {
      try {
        ...
        logger.debug(
          `[ORDER] Moving order ${id} to ${bookSide} side ${ticker[bookSide]}. Old price: ${price}.`,
        );
