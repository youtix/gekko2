import { MissingCandlesBacktestError } from '@errors/backtest/MissingCandlesBacktest.error';
...
export class BacktestStream extends Readable {
  ...
  constructor(daterange: Interval<EpochTimeStamp, EpochTimeStamp>) {
    ...
    this.storage = stateManager.getStorageInstance();
    ...
    if (result?.missingCandleCount) {
      ...
      throw new MissingCandlesBacktestError(daterange);
    }
    ...
    logger.debug(
      `Launching backtest on ${asset}/${currency} from ${toISOString(daterange.start)} -> to ${toISOString(daterange.end)}`,
    );
  }
  ...
  public _read() {
    ...
    if (daterange) {
      ...
      else throw new MissingCandlesBacktestError(daterange);
