...
describe('RealtimeStream', () => {
  ...
  it('should push candles when trades are available', async () => {
    ...
    const { TradeBatcher } = await import('@services/core/batcher/tradeBatcher/tradeBatcher');
    const { CandleManager } = await import('@services/core/candleManager/candleManager');
