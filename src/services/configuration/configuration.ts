...
class Configuration {
  ...
  constructor() {
    ...
    const isJson =
      configFilePath?.endsWith("json5") || configFilePath?.endsWith("json5");
    const isYaml =
      configFilePath?.endsWith("yml") || configFilePath?.endsWith("yaml");
  ...
  public getPlugins() {
    if (!this.configuration)
      throw new MalformedConfigurationError("Empty configuration file");
  ...
  public getStrategy<T>() {
    if (!this.configuration)
      throw new MalformedConfigurationError("Empty configuration file");
    return this.configuration.strategy as T;
  }
  ...
  public getStorage() {
    ...
    if (
      ...
      (watch.mode === "backtest" || find(plugins, { name: "CandleWriter" }))
    ) {
