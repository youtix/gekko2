import { SigmoidLayer } from '@services/learning/layer/sigmoid/sigmoidLayer';
import { Vol } from '@services/learning/volume/vol';
import { describe, expect, it } from 'vitest';

describe('SigmoidLayer', () => {
  // forward: elementwise sigmoid, returns new Vol, input unchanged
  it.each`
    name                              | input
    ${'single zero'}                  | ${[0]}
    ${'mixed small values'}           | ${[1, -2, 0.5, -0.75]}
    ${'near-saturation large values'} | ${[1000, -1000, 5, -5]}
    ${'zeros and decimals'}           | ${[0, 0.25, -0.5, 2]}
  `('forward() elementwise sigmoid: $name', ({ input }) => {
    const v = new Vol(input as number[], 0, 0);
    const layer = new SigmoidLayer({ in_sx: 1, in_sy: 1, in_depth: (input as number[]).length });
    ...
    const expectedOut = (input as number[]).map(x => 1 / (1 + Math.exp(-x)));
  ...
  // backward: d/dx sigmoid(x) = sigmoid(x) * (1 - sigmoid(x)) * upstream
  it.each`
    name                             | input                    | upstream
  ...
  `('backward() gradient: $name', ({ input, upstream }) => {
    ...
    const expectedGrad = (input as number[]).map((x, i) => {
      ...
      return s * (1 - s) * (upstream as number[])[i];
    });
  ...
  `('toJSON/fromJSON: $name', ({ config }) => {
    const layer = new SigmoidLayer(config);
    const json = layer.toJSON();
    const layer2 = new SigmoidLayer({ in_sx: 1, in_sy: 1, in_depth: 1 });
  ...
  it('getParamsAndGrads returns empty array', () => {
    const layer = new SigmoidLayer({ in_sx: 1, in_sy: 1, in_depth: 3 });
